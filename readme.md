# immr26

## Introduction

In this two-day course we provide a comprehensive practical and theoretical introduction to multilevel models, also known as hierarchical or mixed effects models.
We focus primarily on multilevel linear models, though we also cover multilevel generalised linear models.

On Day 1 we begin with random effects models.
These make clear how multilevel models are, in a meaningful sense, models of models, and they provide a solid conceptual foundation for understanding mixed effects models that combine fixed and random components.
Within this treatment of random effects we cover statistical shrinkage in the estimation of group-level effects and the intraclass correlation.
We then move to linear mixed effects models, with particular attention to varying intercept and varying slope regression models.

On Day 2 we extend the linear mixed effects framework to cover multilevel models for nested and crossed data structures, and the incorporation of group-level predictor variables.
We also address how to quantify explained variance in multilevel models and how to determine adequate sample sizes through power analysis.

## Software

Instructions on how to install all the software is [here](software.md).

## Topics

### Topic 1: Random Effects Models

Understand how multilevel models handle variability across groups.
Explore statistical shrinkage and intraclass correlation.

### Topic 2: Normal Random Effects Models

Learn how normal random effects models bridge to linear mixed effects models.
Develop insights into hierarchical data structures.

### Topic 3: Linear Mixed Effects Models

Implement varying intercept and varying slope models.
Gain practical experience fitting and interpreting these models in R.

### Topic 4: Multilevel Models for Nested Data

Learn to model hierarchical data structures.
Understand how nesting affects statistical assumptions and model fit.

### Topic 5: Multilevel Models for Crossed Data

Handle datasets where groups overlap in complex ways.
Compare and contrast nested vs crossed data structures.

### Topic 6: Group-Level Predictors

Incorporate and interpret predictors that vary at the group level.
Learn best practices for handling multilevel covariates.

### Topic 7: Explained Variance in Multilevel Models

Use R-squared and related metrics to evaluate model fit.
Understand how multilevel models partition variance between levels.

### Topic 8: Power Analysis and Sample Size Determination

Learn how to perform power analysis for multilevel studies.
Ensure your sample size is sufficient for reliable inference.

## Guides

Workshop guides and slides are available [here](https://courses.mjandrews.org/immr/).
